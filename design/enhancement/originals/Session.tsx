...
export default function SessionScreen(props: { lang: Lang; setup: Setup; exerciseId: string; demo: boolean; onExit: () => void; onRestart: () => void; onDemo: () => void }) {
  ...
  return <div className={`session ${demo ? "demo-session" : "camera-session"}`} data-stage={stage}>
    <header className="session-header">
      <button className="brand" onClick={onExit} aria-label={t("home")}><span className="brand-mark">ع</span><span className="brand-word">عَزم <bdi>SPARK</bdi></span></button>
      <div className={`session-mode ${demo ? "is-demo" : ""}`}><span />{demo ? c.demo : c.live}</div>
      <button className="text-button" onClick={onExit}>{t("home")}</button>
    </header>
    <main className="session-main">
      ...
      <div className="session-grid">
        <section className={`movement-panel ${demo?'demo-panel':'camera-panel'}`} aria-label={demo?c.demo:c.live}>
          <div className="movement-heading"><span><Icon name={demo?"play":"camera"} size={17}/>{demo?c.guide:c.live}</span><small>{demo?c.demoNotice:t("onDevice")}</small></div>
          {demo?<>
        ...
        <aside className="coach-panel">
          ...
          <div className="session-privacy"><Icon name="shield" size={19}/><p>{demo?c.demoBody:t("privacy")}</p></div>
        </aside>
      </div>
    </main>
    <footer className="session-controls"><button className="stop" ref={stopBtnRef} onClick={stopNow}><Icon name="stop" size={18}/>{t("stop")}</button><button className="ghost sound-button" onClick={()=>setMuted(!muted)} aria-pressed={muted}><Icon name="sound" size={18}/>{muted?t("soundOff"):t("soundOn")}</button><p>{demo?c.demoNotice:c.cameraHelp}</p><span className="control-set">{t("set")} {fmtNum(1,lang)} · {fmtNum(totalReps,lang)} {t("reps")}</span></footer>
    {stage==='rpe'&&<Dialog titleId="rpe-title"><div className="result-symbol"><Icon name="check" size={30}/></div><p className="eyebrow">{demo?c.demoSummary:c.resultIntro}</p><h2 id="rpe-title">{t("rpeTitle")}</h2>{demo&&<p>{c.demoNotSaved}</p>}<div className="rpe-grid">{Array.from({length:11},(_,i)=><button key={i} className={`rpe-btn ${rpe===i?'sel':''}`} aria-pressed={rpe===i} onClick={()=>setRpe(i)}>{fmtNum(i,lang)}</button>)}</div><div className="rpe-labels"><span>{c.easy}</span><span>{c.hard}</span></div>{rpe!==null&&rpe>=8&&<p className="rpe-warn" role="status">{t("rpeHigh")}</p>}<div className="modal-actions"><button className="cta" disabled={rpe===null} onClick={()=>saveAndSummarize(rpe)}>{c.finish}</button><button className="ghost" onClick={()=>saveAndSummarize(null)}>{t("skip")}</button></div></Dialog>}
    {stage==='summary'&&summary&&<Dialog titleId="sum-title"><div className="result-symbol"><Icon name="check" size={30}/></div><p className="eyebrow">{demo?c.demoSummary:c.resultIntro}</p><h2 id="sum-title">{t("summaryTitle")}</h2><p>{def.name[lang]}</p><div className="sum-grid"><div><b>{fmtNum(summary.reps.valid,lang)}</b><span>{t("validReps")}</span></div><div><b>{fmtNum(summary.reps.compensated,lang)}</b><span>{t("compReps")}</span></div><div><b>{fmtNum(summary.reps.partial,lang)}</b><span>{t("partialReps")}</span></div><div><b>{summary.romPct!=null?fmtPct(summary.romPct/100,lang):'—'}</b><span>{t("bestRom")}</span></div></div><p className="micro">{t("ofYourRange")}</p>{summary.rpe!=null&&<p>{t("rpeLabel")}: {fmtNum(summary.rpe,lang)}/{fmtNum(10,lang)}</p>}<p className="sum-note">{demo?c.demoNotSaved:saved?t("saveNote"):c.saveFailed}</p><div className="modal-actions"><button className="cta" onClick={onRestart}>{c.repeat}</button><button className="ghost" onClick={onExit}>{c.newSession}</button></div></Dialog>}
  </div>;
}
