...
export default function SessionScreen(props: { lang: Lang; setup: Setup; exerciseId: string; demo: boolean; preferences: Preferences; onPreferences: (p: Preferences) => void; onExit: () => void; onRestart: () => void; onDemo: () => void }) {
  ...
  const saveAndSummarize = (rpeVal: number | null) => {
    ...
    if (!demo) {
      try {
        ...
        arr.push({ ...s, mode: "camera", setup, moments: P.moments });
  ...
  return <div className={`session ${preferences.focus?"focus-session":""} ${demo ? "demo-session" : "camera-session"}`} data-stage={stage}>
    ...
    <footer className="session-controls"><button className="stop" ref={stopBtnRef} onClick={stopNow}><Icon name="stop" size={18}/>{t("stop")}</button><button className="ghost sound-button" onClick={()=>{setMuted(!muted);onPreferences({...preferences,voice:!muted?'off':'full'});}} aria-pressed={muted}><Icon name="sound" size={18}/>{muted?t("soundOff"):t("soundOn")}</button><p>{demo?c.demoNotice:c.cameraHelp}</p><span className="control-set">{t("set")} {fmtNum(1,lang)} · {fmtNum(totalReps,lang)} {t("reps")}</span></footer>
