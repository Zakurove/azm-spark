...
import { drawDemoScene } from "./demoScene";
...
export function drawOverlay(ctx: CanvasRenderingContext2D, frame: Frame, st: OverlayState): void {
  ...
  if (st.scene) {
    drawDemoScene(ctx, frame, st.scene, st.mirrored);
  } else {
  ...
  for (const i of JOINTS) {
    ...
    const { x, y } = P(i);
    const r = st.scene ? trunkPx * (dim ? 0.025 : 0.036) : dim ? trunkPx * 0.045 : trunkPx * 0.062;
    if (flash) {
      ctx.beginPath();
