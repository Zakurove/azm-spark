...
export default function App(){
 ...
 const [preferences,setPreferences]=useState(readPreferences),[settings,setSettings]=useState(false),[records,setRecords]=useState<SavedSession[]>([]),[run,setRun]=useState<WorkoutRun|null>(null),[busy,setBusy]=useState(false),[demo,setDemo]=useState(qs.get('demo')==='1'&&qs.get('autostart')==='1'),[authView,setAuthView]=useState(qs.get('app')==='1');
 ...
 if(!account&&!authView)return <Landing lang={lang} onLanguage={()=>setLang(lang==='ar'?'en':'ar')} onEnter={()=>setAuthView(true)} onDemo={()=>setDemo(true)}/>;
 if(!account)return <Auth lang={lang} onLanguage={()=>setLang(lang==='ar'?'en':'ar')} onSuccess={setAccount} onDemo={()=>setDemo(true)}/>;
