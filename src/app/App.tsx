...
export default function App(){
 ...
 if(!account&&!authView)return <Landing lang={lang} onLanguage={()=>setLang(lang==='ar'?'en':'ar')} onEnter={register=>{setAuthRegister(register===true);setAuthView(true);}} onDemo={()=>setDemo(true)}/>;
 if(!account)return <Auth lang={lang} onLanguage={()=>setLang(lang==='ar'?'en':'ar')} onSuccess={setAccount} onDemo={()=>setDemo(true)} initialRegister={authRegister} onBack={()=>{setAuthView(false);history.replaceState({},'','/');}}/>;
